...
def main(args):
    ...
    pd.DataFrame.to_csv(
        df, output_folder + "/feature_importance.txt", sep="\t", index=False
    )

    if model_type == "XGB_class":
        # AUC plot
        plt.plot(
            [0, 1], [0, 1], linestyle="--", lw=2, color="r", label="Luck", alpha=0.8
        )
        ...
        print(
            f"***  Median Average Precision_score was {median_avg_precision_score}  ***"
        )

        # save results per shuffle
        rez = pd.DataFrame(
            np.column_stack([roc_auc_scores, f1s, avg_precision_scores]),
        ...
        rr = pd.DataFrame(
            np.column_stack(
                [median_roc_auc_score, median_f1, median_avg_precision_score]
            ),
            columns=["Median AUC", "Median F1-score", "Median Avg. Precision Score"],
        )
        pd.DataFrame.to_csv(
            rr,
            output_folder + "/aggregated_metrics_classification.txt",
            sep="\t",
            index=False,
        )
    else:
        ...
        pd.DataFrame.to_csv(
            rr,
            output_folder + "/aggregated_metrics_regression.txt",
            sep="\t",
            index=False,
        )
    ...
    if tune_parameters:
        best_parameters_file = open(
            output_folder + "_best_parameters.txt", "w", encoding="utf-8"
        )
