...
for (i in 1:nrow(pred_sex_composition)) {
  ...
  m2 <- lm(logdep ~ Sex + Age + BMI + HT + DM + CurrSmoking + Alcohol + TotalCalories + 
...
openxlsx::write.xlsx(res2, "results/lm/lm_sex_metagen.xlsx")
...
# Make forest plot
(forest_plot_sex <- ggplot(res3, aes(x = est, y = microbe, color = model, shape = sigq)) +
...
ggsave("results/lm/lm_metagen_sex.pdf", width = 10, height = 8)
