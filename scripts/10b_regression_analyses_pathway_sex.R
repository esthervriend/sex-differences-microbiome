# Regression analyses pathways - menopause
...
# Best predictors
pred_menopause_pathways <- rio::import("data/menopause_pathways_feature_importance.txt") %>% slice_head(n = 20)

# Regression analyses menopause
res <- c()
for (i in 1:nrow(pred_menopause_pathways)) {
    var_name <- pred_menopause_pathways$FeatName[i]
    ...
    # run models
    m0 <- lm(dep ~ MenopauseYn, data = dfcomplete)
    m1 <- lm(dep ~ MenopauseYn + Age + BMI + HT + DM + CurrSmoking, data = dfcomplete)
    m2 <- lm(dep ~ MenopauseYn + Age + BMI + HT + DM + CurrSmoking + Alcohol + TotalCalories + 
                 Fibre + Proteins, data = dfcomplete)
    
    # extract estimates for variable menopause
...
openxlsx::write.xlsx(res2, "results/lm/lm_menopause_pathways.xlsx")
...
# Make forest plot
(forest_path_menopause <- ggplot(res3, aes(x = est, y = pathway, color = model, shape = sigq)) +
        ...
        labs(title = "Best predicting pathways for menopause",
             x = "Estimate and 95% CI for postmenopausal women", y = "", color = "", shape = "") + 
        theme_Publication()
)
ggsave("results/lm/lm_pathways_menopause.pdf", width = 12, height = 8)
